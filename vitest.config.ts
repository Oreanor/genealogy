import { defineConfig } from 'vitest/config';
import react from '@vitejs/plugin-react';
import path from 'path';

export default defineConfig({
  plugins: [react()],
  resolve: {
    alias: {
      '@': path.resolve(__dirname, './src'),
    },
  },
  test: {
    environment: 'happy-dom',
    pool: 'threads',
    setupFiles: ['./vitest.setup.ts'],
    include: ['src/**/*.{test,spec}.{ts,tsx}'],
    coverage: {
      provider: 'v8',
      reporter: ['text', 'html', 'lcov'],
      include: ['src/**/*.{ts,tsx}'],
      // Target ~80–85% on lines/stmts/funcs; branches stay lower (UI branching).
      thresholds: {
        lines: 82,
        statements: 80,
        branches: 66,
        functions: 78,
      },
      exclude: [
        'src/**/*.d.ts',
        'src/**/*.test.{ts,tsx}',
        'src/**/*.spec.{ts,tsx}',
        'next-env.d.ts',
        '**/index.ts',
        'src/app/**',
        'src/lib/types/**',
        'src/proxy.ts',
        'src/components/SetDocumentLang.tsx',
        'src/components/LocalePreferenceRedirect.tsx',
        'src/components/ui/PageColorPickerClient.tsx',
        'src/components/ui/PageColorPicker.tsx',
        'src/components/ui/LocaleSwitcher.tsx',
        // Heavy UI / hard to unit-test; keep coverage threshold on core lib and key components
        'src/components/admin/**',
        'src/components/book/BookView.tsx',
        'src/components/StorageGate.tsx',
        'src/components/ui/AdminButton.tsx',
        'src/components/ui/ImageLightbox.tsx',
        'src/lib/utils/color.ts',
        'src/components/book/BookToolbar.tsx',
        'src/components/content/PageContentRenderer.tsx',
        // Leaflet / map glue: low ROI for unit tests
        'src/components/book/CityPreviewMap.tsx',
        'src/components/book/YearArchiveRangeSlider.tsx',
        'src/components/book/IndexedEventsMapSection.tsx',
        'src/components/book/useLeafletBookMap.ts',
        'src/components/book/usePrizyvPointsMap.ts',
        'src/components/book/useIndexedEventsMap.ts',
        'src/lib/utils/leafletMap.ts',
        'src/lib/utils/tree.ts',
        'src/components/tree/useTreePan.ts',
        'src/components/tree/useTreeSegments.ts',
        'src/components/ui/Tooltip.tsx',
        'src/components/ui/molecules/LoadingOverlay.tsx',
        'src/lib/i18n/types.ts',
      ],
    },
    globals: true,
  },
});
